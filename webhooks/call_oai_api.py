...
def oai_respond(message, init_append_msg='', extra_msgs: list = []):
    response = openai.ChatCompletion.create(
        ...
        messages=[{"role": "system",
                   "content": f"有一个学习交流网站名叫交大门，上面的用户主要是西安交通大学（简称西安交大或交大）的学生。你是这个网站上的一名普通用户，你的主要工作是和其他用户交流，解决问题，提供信息，等等。你很友善并且回答得很精炼、基于事实。你的言论不得违反中华人民共和国法律，不得有伤风化，不得做人身攻击。你主要使用简体中文做回答。不要重复之前的信息。{init_append_msg}"}, *extra_msgs,
